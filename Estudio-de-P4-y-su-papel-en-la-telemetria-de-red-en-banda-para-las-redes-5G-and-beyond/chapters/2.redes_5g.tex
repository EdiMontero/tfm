...
\item \textbf{Visión de 6G}: Redes ultra confiables, latencia casi nula y capacidades de inteligencia artificial integradas de manera nativa \cite{saad20206g,dang2020should}.
...
3GPP \cite{3gpp.23.799,3gpp.sp.160455} introdujo 6 opciones de implementación para la tecnología New Radio (NR) como se muestra en la figura \ref{fig:nr-arch}. Estas opciones se dividen en dos categorías principales: \textbf{Non-Standalone (NSA)} y \textbf{Standalone (SA)}. 
...
La arquitectura 5G Non-Standalone (NSA) es una configuración de red que permite la coexistencia y colaboración entre las tecnologías 4G LTE y 5G NR. En esta arquitectura, la red 4G LTE actúa como la red principal para la señalización y el control, mientras que la red 5G NR se utiliza principalmente para el transporte de datos de alta velocidad. Esta coexcistancia se logra mediante \textbf{Multi-Radio Dual Connectivity (MR-DC)} \cite{3gpp.ts.37.340}, descrito mas adelante y que permite a los dispositivos de usuario (UE) conectarse simultáneamente a dos distintas generaciones de red de radio. 
Las opciones pueden ser:
\begin{itemize}
    \item \textbf{EN-DC (E-UTRA-NR Dual Connectivity)}: \emph{Opcion 3}, donde el UE se conecta a una estación base LTE (eNB) como nodo maestro y a una estación base NR (gNB) como nodo secundario. El eNB gestiona la señalización y el control, mientras que el gNB se utiliza principalmente para la transferencia de datos de alta velocidad.
    \item \textbf{NGEN-DC (NG-RAN E-UTRA-NR Dual Connectivity)}: \emph{Opcion 7}, similar a la opcion 3, pero con una integración más estrecha entre las estaciones base LTE y NR. En este modo, el eNB y el gNB están conectados a través de la interfaz Xn, lo que permite una mejor coordinación y gestión de recursos entre ambas tecnologías.
    ...
    \item \textbf{NR-DC (NR-NR Dual Connectivity)}: \emph{Opcion 2}, En la que UE se conecta a dos estaciones base NR (gNB) como nodos maestro y secundario. Ambas estaciones base están conectadas a través de la interfaz Xn, lo que permite una coordinación y gestión de recursos más eficiente entre ellas.
\end{itemize}
...
\end{figure}
%-------------------------------
Para más detalles de los acrónimos y protocolos, consultar la lista de acrónimos al inicio del documento.
...
\begin{itemize}
    ...
    \item \textbf{N2}: Conecta la estación base 5G (gNB) con el AMF en el plano de control. Utiliza el protocolo NGAP (Next Generation Application Protocol) \cite{3gpp.ts.38.401} para la señalización y gestión de la conexión entre el gNB y el AMF.
    \item \textbf{N3}: Esta interfaz conecta la estación base 5G (gNB) con el User Plane Function (UPF) en el plano de usuario. Utiliza el protocolo GTP-U (GPRS Tunneling Protocol - User Plane) para la transmisión de datos entre el gNB y el UPF.
    \item \textbf{N4}: Conecta el SMF (Session Management Function) con el UPF en el plano de control. Utiliza el protocolo PFCP (Packet Forwarding Control Protocol) \cite{3gpp.29.244} para la gestión y configuración del plano de usuario en el UPF.
