...
\subsection{Creacion de la base de datos InfluxDB}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/creacion-influxdb.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/influxdb-connection.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/influxdb-connection-2.png}
...
\subsubsection{Detección automática de endianness}
...
El script implementa un sistema de \textbf{auto-detección heurística} que evalúa ambos órdenes de bytes y selecciona el más plausible mediante un sistema de puntuación:
...
\begin{lstlisting}[language=Python,
    ...
    literate=
      ...
      {Á}{{\'A}}1 {É}{{\'E}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {Ú}{{\'U}}1
      {ñ}{{\~n}}1 {Ñ}{{\~N}}1,
    caption={Sistema de auto-detección de endianness}, 
    label=lst:int-script-endian]
def _score_int_md(values):
    """Heurística para determinar el endianness más plausible.
    
    Los switch_id y ports son enteros pequeños (1..9). Cuando el orden
    de bytes es incorrecto, se convierten en valores como 256, 512, etc.
    """
    score = 0
    ...
    # Switch IDs son enteros pequeños en este proyecto
    if 1 <= switch_id <= 32:
        score += 5
    elif switch_id in (256, 512, 768, 1024):
        score -= 5
    
    # Puertos también son pequeños
    ...
    # enq_qdepth no debe parecer un byte desplazado (0xNN000000)
...
def _decode_int_md(md_data, endian):
    """Decodificar bloque INT-MD usando endian '<' o '>'."""
...
El sistema de puntuación evalúa la plausibilidad de los valores decodificados. Por ejemplo, si al decodificar con big-endian obtenemos \texttt{switch\_id = 256} pero con little-endian obtenemos \texttt{switch\_id = 1}, el sistema asignará mayor puntuación a little-endian, ya que es más razonable tener un switch con ID 1 que con ID 256 en una topología pequeña.
...
La recuperación heurística de errores es particularmente importante cuando se detectan anomalías en los datos. Por ejemplo, si \texttt{hop\_latency\_field = 0} y \texttt{egress\_timestamp = 0} pero \texttt{queue\_occupancy} contiene un valor que parece un timestamp, el script intenta intercambiar los campos para recuperar los valores correctos.
...
def handle_packet(pkt):
    ...
    print(f" E2E latency (sum): {e2e_latency_ms:.3f}ms ({e2e_latency_us}µs)")
\end{lstlisting}
...
def main():
    ...
    print(f"Conversión: 1 ms = {US_TO_MS:,.0f} µs")
    print("=" * 50)
