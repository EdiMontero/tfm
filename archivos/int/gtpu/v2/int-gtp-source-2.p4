...
parser MyParser(packet_in packet,
    ...
    state parse_udp {
        ...
        // Check if this is GTP-U traffic using transition select
...
control MyIngress(inout headers hdr,
    ...
    action do_insert_int() {
        ...
        hdr.int_shim.length = 44;  // Will be updated by transit switches
    ...
    // Table 1: Check if source IP is configured for INT
    table int_source_ip_table {
        key = {
            hdr.ipv4.srcAddr: lpm;  // Use LPM for flexibility (can use exact too)
        }
        actions = {
            int_source_action;
            NoAction;
    ...
    // Table 2: Check traffic type for IPs marked for INT
    ...
    apply {
        ...
        if (hdr.ethernet.isValid() && hdr.ipv4.isValid()) {
            // First: Check if source IP is configured for INT monitoring
            if (hdr.ipv4.srcAddr != 0) {  // Ensure we have a valid source IP
                int_source_ip_table.apply();
            }

            // Second: If source IP matched, check if traffic type matches
            if (meta.should_insert_int == 1) {
                if ((hdr.ipv4.protocol == PROTOCOL_SCTP && hdr.sctp.isValid()) ||
                    ...
                    // Not SCTP or UDP, clear the INT flag
                    meta.should_insert_int = 0;
                }
            }
