...
parser MyParser(packet_in packet,
    ...
    state parse_udp {
        ...
        transition select(hdr.udp.dst_port == GTP_U_PORT || hdr.udp.src_port == GTP_U_PORT) {
            true: parse_gtp_u;
            false: accept;
        }
    }
    ...
    state parse_int_header {
        ...
        // Calculate how many metadata blocks to extract (SW1 + SW2 = 2 blocks)
    ...
    state parse_int_metadata {
        // Extract ALL existing metadata blocks (SW1 + SW2)
        packet.extract(hdr.int_md[0]);  // SW1 data
        packet.extract(hdr.int_md[1]);  // SW2 data
    ...
    state parse_encapsulated_udp {
        packet.extract(hdr.udp);
        transition select(hdr.udp.dst_port == GTP_U_PORT || hdr.udp.src_port == GTP_U_PORT) {
            true: parse_encapsulated_gtp_u;
            false: accept;
        }
    }

    state parse_encapsulated_gtp_u {
        packet.extract(hdr.gtp_u);
        transition accept;
    }
    ...
    apply {
        ...
        if (hdr.ethernet.isValid()) {
            if (hdr.int_shim.isValid()) {
                // Add SW3 metadata in the next available position (int_md[2])
                ...
                // Always activate sink in SW3 (last hop)
...
control MyEgress(inout headers hdr,
    ...
    action update_int_metadata() {
        // Ensure int_md[2] is valid before updating
        ...
        // Update egress fields
        hdr.int_md[2].egress_port_id = (bit<16>)standard_metadata.egress_port;
        hdr.int_md[2].egress_timestamp = (bit<32>)standard_metadata.egress_global_timestamp;
        hdr.int_md[2].hop_latency = hdr.int_md[2].egress_timestamp - hdr.int_md[2].ingress_timestamp;
        hdr.int_md[2].queue_occupancy = (bit<32>)standard_metadata.enq_qdepth;
    }
    ...
    action strip_int_headers() {
        ...
        // Invalidate all INT metadata blocks manually
    ...
    apply {
        if (hdr.int_shim.isValid()) {
            // Always update INT metadata first
            update_int_metadata();

            // Check if this is a cloned packet (instance_type == 1 for I2E clone)
            // OR if going to port 9 (INT collector)
            if (standard_metadata.instance_type == 1 || standard_metadata.egress_port == 9) {
                // Cloned packet to INT collector - keep INT headers intact
                // Do nothing, keep all INT headers
            } else {
                // Original packet to destination host - strip INT headers
...
control MyDeparser(packet_out packet, in headers hdr) {
    apply {
        ...
        // Emit all INT headers always
        // BMv2 will automatically handle invalid headers (won't include them in packet)
        packet.emit(hdr.int_shim);
        packet.emit(hdr.int_header);

        // Emit all metadata blocks (BMv2 will automatically skip invalid ones)
