        ...
        if (hdr.ethernet.isValid()) {
            if (hdr.int_shim.isValid()) {
                ...
                if (hdr.int_header.remaining_hop_cnt > 0) {
                    ...
                    hdr.int_md[2].switch_id = meta.switch_id;
...
control MyEgress(inout headers hdr,
    ...
    apply {
        if (hdr.int_shim.isValid()) {
            ...
            if (standard_metadata.egress_port == meta.clone_port) {
                // Port 9 (report server) - keep INT headers intact, already updated
                // Do nothing else, keep all INT headers
            } else {
                // For other ports (destination host) - strip INT headers
